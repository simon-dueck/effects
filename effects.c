...
typedef struct Clean {
    uint32_t (*apply)(uint32_t, union Effect *);
};

typedef struct Delay {
    uint32_t buffer[DELAY_MAX];
    uint32_t counter; // = 0;
    uint32_t depth;// = 100000; //default starting delay is 100000 is 0.5 sec approx
    uint32_t (*apply)(uint32_t, union Effect *);
};

typedef struct Bitcrusher {
    uint32_t value;
    uint32_t (*apply)(uint32_t, union Effect *);
};
...
typedef struct Looper {
    uint32_t buffer[LOOP_MAX];
    uint32_t counter;// = 100;
    uint32_t depth;// = 100000; //default starting delay is 100000 is 0.5 sec approx.
    uint32_t (*apply)(uint32_t, union Effect *);
};

typedef struct Octaver {
    uint32_t buffer[OCTAVE_MAX];
    uint32_t write;// = 0;
    uint32_t read;// = 0;
    uint32_t value;// = 1;
    uint32_t depth;// = 50000; //default starting delay is 100000 is 0.25 sec approx.
    uint32_t divider;
    uint32_t (*apply)(uint32_t, union Effect *);
};

typedef struct Reverb {
    uint32_t buffer_1[REVERB_MAX];
    uint32_t buffer_2[REVERB_MAX];
    uint32_t buffer_3[REVERB_MAX];
    uint32_t counter_1;// =0
    uint32_t counter_2;
    uint32_t counter_3;
    uint32_t depth_1; // = 10000
    uint32_t depth_2; // = 5000
    uint32_t depth_3; // = 2500
    uint32_t (*apply)(uint32_t, union Effect *);
};
...
typedef struct Tremolo {
    ...
    uint32_t waveform[1000];/* = {0x800,0x80c,0x819,0x826,0x833,0x840,0x84d,0x85a,0x866,0x873,0x880,0x88d,0x89a,0x8a7,0x8b3,0x8c0,
...
struct Delay * new_delay(uint32_t depth) {
...
struct Bitcrusher new_bitcrusher(uint32_t bits) {
    struct Bitcrusher b;
    b.value = bits;
    return b;
};

uint32_t delay(uint32_t input_signal, union Effect * effect) {

    struct Delay d = (effect.delay)*;
	...
	return (Delay_Buffer[DelayCounter] + input_signal) >> 1;

};

uint32_t bitcrusher(uint32_t input_signal, union Effect * effect) {
    return input_signal << bitcrushing_value;
};

uint32_t booster(uint32_t input_signal, union Effect * effect) {};

uint32_t distortion(uint32_t input_signal, union Effect * effect) {};

uint32_t echo(uint32_t input_signal, union Effect * effect) {};

uint32_t fuzz(uint32_t input_signal, union Effect * effect) {};

uint32_t looper(uint32_t input_signal, union Effect * effect) {};

uint32_t octaver(uint32_t input_signal, union Effect * effect) {};

uint32_t reverb(uint32_t input_signal, union Effect * effect) {};

uint32_t tremolo(uint32_t input_signal, union Effect * effect) {};
